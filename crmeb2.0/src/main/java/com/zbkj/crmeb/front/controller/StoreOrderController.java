...
public class StoreOrderController {
     ...
     * @return 删除结果
     */
    @ApiOperation(value = "删除订单",notes = Constants.DELETE)
     ...
     * @return 退款理由
     */
    @ApiOperation(value = "订单退款理由",notes = Constants.SELECT)
     ...
     * @return 物流信息
     */
    @ApiOperation(value = "物流信息查询",notes = Constants.SELECT)
    ...
    @ApiOperation(value = "待评价商品信息查询",notes = Constants.SELECT)
    ...
    @ApiOperation(value = "订单流程是否需要弹窗",notes = Constants.SELECT)
    @RequestMapping(value = "/orderProcedureIsPopup", method = RequestMethod.GET)
    @ApiImplicitParam(name = "uid", value = "该订单推荐人用户ID标识", required = true)
    public CommonResult<OrderProcedureIsPopupResponse> orderProcedureIsPopup(@RequestParam("uid")Integer uid){
        return CommonResult.success(orderService.orderProcedureIsPopup(uid));
    }
