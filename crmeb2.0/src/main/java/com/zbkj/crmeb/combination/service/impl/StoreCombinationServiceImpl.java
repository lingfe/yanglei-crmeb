...
public class StoreCombinationServiceImpl extends ServiceImpl<StoreCombinationDao, StoreCombination> implements StoreCombinationService {
    ...
    public CombinationDetailResponse getH5Detail(Integer comId) {
        ...
        BeanUtils.copyProperties(storeCombination, infoResponse);//将-拼团商品信息-转换为-拼团商品详情H5
        ...
        infoResponse.setContent(ObjectUtil.isNull(sd.getDescription()) ? "" : sd.getDescription()); //拼团商品详情-从主商品详情复制
        infoResponse.setSales(storeProduct.getSales());// 拼团销量 = 原商品销量（包含虚拟销量）
        infoResponse.setFicti(storeProduct.getFicti());// 虚拟销量-从主商品取
