...
public class WeChatPayServiceImpl implements WeChatPayService {
    ...
    public Map<String, String> unifiedorder(StoreOrder storeOrder, String ip) {
        ...
        if (storeOrder.getIsChannel() == 2 ||
                ...
                storeOrder.getIsChannel() == 5) {// H5、微信AppIos、微信App安卓
            userToken.setToken("");
        }
        ...
        if (storeOrder.getIsChannel() == 4 || storeOrder.getIsChannel() == 5) {// 微信AppIo 或 微信App安卓
        ...
        // 预下单
        CreateOrderResponseVo responseVo = unifiedOrder(unifiedorderVo); // 微信预下单
    ...
    public Boolean queryPayResult(String orderNo) {
        ...
        if (pre.equals("order")) {// 支付订单
            ...
            //验证支付类型
            logger.info("*********验证支付类型********"+storeOrder.getPayType());
            switch (storeOrder.getPayType()){
                case PayConstants.PAY_TYPE_WE_CHAT:
                    ...
                    if (storeOrder.getIsChannel() == 1) {// 小程序
                        logger.info("*********小程序********"+storeOrder.getPayType());
                        appId = systemConfigService.getValueByKeyException(Constants.CONFIG_KEY_PAY_ROUTINE_APP_ID);
                        mchId = systemConfigService.getValueByKeyException(Constants.CONFIG_KEY_PAY_ROUTINE_MCH_ID);
                        signKey = systemConfigService.getValueByKeyException(Constants.CONFIG_KEY_PAY_ROUTINE_APP_KEY);
                    }
                    ...
                    try{
                        ...
                        if(response.isSuccess()){
                            ...
                            if(!"TRADE_SUCCESS".equals(response.getTradeStatus())){
                                throw new CrmebException("支付未成功！");
                            }
                        } else {
                        ...
                        throw new CrmebException("支付宝支付订单查询错误！"+e.getMessage());
                    }
                    break;
                default:
                    throw new CrmebException("不是微信或支付宝支付类型订单，请重新选择支付方式");
            }

            //得到-支付用户信息
            logger.info("*********得到-支付用户信息********");
            ...
            //验证执行更新结果
            logger.info("*********验证执行更新结果********"+updatePaid);
            if (!updatePaid) {
                throw new CrmebException("支付成功更新订单失败");
            }

            // 添加支付成功task
            logger.info("*********添加支付成功task********"+updatePaid);
            redisUtil.lPush(Constants.ORDER_TASK_PAY_SUCCESS_AFTER, storeOrder.getId()); // 查看支付结果
            return Boolean.TRUE;
        }else{
            // 充值订单
